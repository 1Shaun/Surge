			...
			if (resp.statusCode == 403) {
				...
				if (obj && obj.errno == 0) {
					if (obj.data.sign.sign) {
						...
						$.detail += "签到获得 " + todayearn + " 福气金，";
					} else {
						$.subTitle += "福气金🔄";
					}
					let total = Number(obj.data.welfare.carousel_text[0].slice(4));
					$.detail += "账户共有 " + total + " 福气金，可抵扣 " + total / 100 + " 元。";
...
async function reward() {
	...
	if ($.rewardtotal) $.detail += "捡回遗忘的 " + $.rewardtotal.toFixed(2) + " 元福气金。";
}
	...
	return $.get({
		...
		.then((resp) => {
			...
			if (obj.errno == 0) {
				...
				$.error(resp.body);
				throw new ERR.BodyError(
					"打车后未领福气金列表返回错误，请在 BoxJs 中开启调试模式运行后反馈日志。\n" +
						obj.errmsg
				);
			}
		})
		...
		await $.get({
			...
			.then((resp) => {
				...
				if (obj.errno == 0) {
					...
					$.error(resp.body);
					throw new ERR.BodyError(
						"打车后未领福气金领取返回错误，请在 BoxJs 中开启调试模式运行后反馈日志。\n" +
							obj.errmsg
					);
				}
			})
		...
		.then((resp) => {
			...
			if (obj.errno == 0) {
				if (obj.data.bonus_amount) {
					...
					$.detail += "上传 " + $.todayStep + " 步，已领取 " + stepBonus + " 福气金。";
				} else {
					$.detail += "步数达标福气金" + obj.data.message_text + "。";
				}
			} else {
		...
		.then((resp) => {
			...
			if (obj.errno == 0) {
				$.detail +=
					"\n" +
					(obj.data.type == 1
						? "明日步数赛报名花费 " + $.stepMatchJoinBonus + " 福气金。"
